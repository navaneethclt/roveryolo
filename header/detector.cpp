...
#ifdef _WIN32
    std::wstring w_modelPath = utilsm::charToWstring(modelPath.c_str());
...
void YOLODetector::preprocessing(cv::Mat &image, float*& blob, std::vector<int64_t>& inputTensorShape)
{
    ...
    utilsm::letterbox(resizedImage, resizedImage, this->inputImageShape,
                                                    ...
                                                    const float& confThreshold, const float& iouThreshold)
{
    ...
    for (int idx : indices)
    {
        ...
        utilsm::scaleCoords(resizedImageShape, det.box, originalImageShape);
                                            ...
                                            const float& iouThreshold = 0.45)
{
    ...
    size_t inputTensorSize = utilsm::vectorProduct(inputTensorShape);
   ...
   /* std::cout << "Input name: " << inputNames << std::endl;
    ...
    std::vector<Ort::Value> outputTensors = this->session.Run(Ort::RunOptions{nullptr}, inputNames,
                                                              inputTensors.data(),
                                                              1,
                                                              outputNames,
                                                              1);
